...
- 
    ...
    */
    ...
    ```
     try (Connection conn = DBConnection.getConnection();
         ...
         System.out.printf("%-10s %-20s %-15s %-15s%n",
                 "Store ID", "Store Name", "City", "Manager");
         System.out.println("----------------------------------------------------------------------");
    ...
    // ============================================================
    // 2. 도시별 매장 조회 (REQ6: user input + join + view)
    // ============================================================
    ...
    ```
     // view_order_detail 뷰를 활용하여 매장 정보 + 주문 건수 함께 조회
     ...
     try (Connection conn = DBConnection.getConnection();
         ...
         System.out.printf("%-10s %-20s %-15s %-15s %-10s%n",
                 "Store ID", "Store Name", "City", "Manager", "Orders");
         System.out.println("----------------------------------------------------------------------");
    ...
    // ============================================================
    // 3. 도시별 매장 수 분석 (REQ7: aggregation + group by)
    // ============================================================
    private void countStoresByCity() {
    String sql = "SELECT city, COUNT(*) AS store_count " +
    "FROM store " +
    "GROUP BY city " +
    ...
    // ============================================================
    // 4. 신규 매장 추가 (REQ5: insert + user input)
    // ============================================================
    ...
    ```
     ...
     try (Connection conn = DBConnection.getConnection();
         ...
         if (rows > 0) {
             System.out.println("✓ 매장 '" + storeName + "'이(가) 성공적으로 추가되었습니다.");
         }
    ...
    // ============================================================
    // 5. 매장 정보 수정 (REQ8: update + user input, REQ12: transaction)
    // ============================================================
    ...
    ```
     ...
     try {
         ...
         conn.setAutoCommit(false); // REQ12: 트랜잭션 시작
         ...
         if (rows > 0) {
             conn.commit(); // 트랜잭션 커밋
             System.out.println("✓ 매장 정보가 성공적으로 수정되었습니다.");
         } else {
             conn.rollback();
    ...
    // ============================================================
    // 6. 매장 삭제 (REQ9: delete + user input)
    // ============================================================
    ...
    ```
     ...
     try (Connection conn = DBConnection.getConnection();
         ...
         if (rows > 0) {
             System.out.println("✓ 매장이 성공적으로 삭제되었습니다.");
         } else {
             System.out.println("해당 ID의 매장을 찾을 수 없습니다.");
         }
